...
export function GithubLogoInline({ href }: { href: string; }) {
    return (
        <a href={href} target="_blank" title="Open the source code of the project on GitHub">
            <div className="w-4 h-4 p-px text-purple-500 border-purple-400 border rounded">
                <GitHubSvg />
            </div>
        </a>
    );
...
function GithubLogoBottomRight({ href }: { href: string; }) {
    return (
        <div className="fixed flex-none flex right-2 bottom-3">
            <a
                className="px-1 pt-2 pb-0 text-gray-500 border-gray-500 border rounded-full"
